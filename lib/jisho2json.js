...
function init() {
  ...
  document.body.addEventListener('keyup', ifKeyDo(_constants.ESC_KEYCODE, toaster.hide));
  document.body.addEventListener('click', onEntryClick);
}

function buildEntries(element) {
  var entries = (0, _domHelpers.qsa)(SELECTORS.MEANING, element).map(function (el) {
    return buildEntry({
      element: element,
      tags: (0, _domHelpers.findSibling)(SELECTORS.TAGS, el),
      counter: (0, _domHelpers.qs)(SELECTORS.MEANING_COUNTER, el), // if no counter, it's a nonstandard meaning/info thin,
      definition: (0, _domHelpers.qs)(SELECTORS.MEANING_TEXT, el),
      info: (0, _domHelpers.qsa)(SELECTORS.MEANING_INFO, el),
      sentence: (0, _domHelpers.qs)(SELECTORS.MEANING_SENTENCE, el)
    });
  });
...
function buildEntry(_ref3) {
  ...
  var ordinal = parseInt(getCleanText(counter), 10);
  var lexemes = getCleanText(definition).split('; ');

  var type = function () {
    var isWikipedia = tags.innerText.includes('Wikipedia');
    var isOther = tags.innerText.includes('Other forms');
    switch (true) {
      case isWikipedia:
        return 'wiki';
      case isOther:
        return 'other';
      case !Number.isNaN(ordinal):
        return 'standard';
      default:
        return 'unknown';
    }
...
function parseSentence(element) {
  if (element) {
    var en = (0, _domHelpers.qs)(SELECTORS.SENTENCE_EN, element);
    var ja = (0, _domHelpers.qsa)(SELECTORS.SENTENCE_JA_MORPHEMES, element).map(function (node) {
...
function buildReading(element) {
  var character = getCleanText((0, _domHelpers.qs)(SELECTORS.READING_CHARACTER, element));
  var kanaEl = (0, _domHelpers.qs)(SELECTORS.READING_KANA, element);
  var kana = getCleanText(kanaEl);
...
function getCleanText(input) {
  ...
  var isNode = input.nodeType === 1;
  var isTextNode = input.nodetype === 3;
  var text = (isNode && input.innerText || isTextNode && input.textContent || input).trim();
  return (0, _smartQuotes2.default)(text);
}
