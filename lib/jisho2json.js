...
function init() {
  ...
  function getEntry({ target }) {
    const isEntryLinks = getParent(target, '.concept_light-status', '#primary');
    if (isEntryLinks) return false;

    const desiredTarget = '.concept_light';
    const traverseLimit = document.querySelector('#primary');
    const entry = getParent(target, desiredTarget, traverseLimit);
  ...
  // bind shortcut key for quick re-searches
  document.querySelector('body').addEventListener('keypress', focusSearchInput);

  // parse and copy entry to clipboard
  document.querySelector('body').addEventListener('click', getEntry);

  console.info('jisho2json loaded');
}

function focusSearchInput({ target, which } = event) {
  const TILDE_KEYCODE = 96;
  const SEARCH_INPUT = '#keyword';
  const isNotInput = !target.matches(SEARCH_INPUT);
  const isKeyTilde = which === TILDE_KEYCODE;
  const searchElement = document.querySelector(SEARCH_INPUT);
  if (isKeyTilde && isNotInput) {
    searchElement.value = "";
    searchElement.focus();
...
/**
 * Walks up DOM tree to find desired element for a given selector (stops at limit)
 * @param  {HTMLElement} initial starting DOM node
 * @param  {String} selector desired element
 * @param  {HTMLElement} limit element to stop at
 * @return {HTMLElement} Parent element or false
 */
function getParent(initial, selector, limit) {
  if (initial.nodeType === 1 /* it's an element! */ && initial.matches(selector)) {
    return initial;
  }
  if (initial === limit || !initial.parentNode) {
    return false;
  }
  return initial.parentNode ? getParent(initial.parentNode, selector) : false;
}
